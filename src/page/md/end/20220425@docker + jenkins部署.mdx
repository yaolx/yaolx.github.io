...
- 配置 ssh 信息
  ...
  一直回车，id_rsa 创建成功
  再通过 cat id_rsa.pub 复制该秘钥

  后续将它复制到 ssh 服务器那边的 authorized_keys 里，方便后续免密登录
...
- 进入 jenkins 容器：ssh root@192.168.99.100 -p 2222
  要求输入密码，验证通过
- pc 主机验证，用 secureCRT 连接，也是输入密码，验证通过

9. 在 ssh 容器中，创建允许外部访问的认证文件（TO 免密功能还待处理）
